...
- `storage.rules` **validado via emulador local** numa segunda tentativa (a primeira travou baixando `cloud-storage-rules-runtime-v1.1.3.jar`; limpei processos órfãos de tentativas anteriores que estavam prendendo as portas 9199/4000/4400/4500 e rodei de novo): `firebase emulators:start --only firestore,storage --project myappcar-cd4ec` subiu com **"All emulators ready!"**, Storage respondendo em 127.0.0.1:9199, sem erro de sintaxe/compilação. **Validação feita foi só de sintaxe/compilação**, não de comportamento (não testei upload real como usuário Premium x gratuito contra o emulador).
- **`storage.rules` e `firestore.rules` publicados manualmente pelo usuário no console do Firebase** (Storage → Rules e Firestore Database → Regras) em 2026-09-22 — ambos no ar no projeto `myappcar-cd4ec`.
  - Na primeira tentativa, a função `isPremium` do `firestore.rules` (com `let`) foi **rejeitada pelo console** ("Unexpected ';'", "mismatched input 'let' expecting '}'"), mesmo já validada sem erro no emulador local — divergência real entre o parser do console e o do emulador para essa construção específica. Reescrita sem `let`, usando duas funções auxiliares (`entitlementActive`/`betaPremiumActive`) chamadas por `isPremium` — revalidada limpa no emulador e publicada com sucesso. **Se mexer de novo nesse arquivo, evite `let` dentro de `function` — prefira funções auxiliares.**
  - Numa colagem anterior, um erro diferente ("Unexpected 'cd'", "mismatched input 'service'") veio de colar por cima do conteúdo antigo sem apagar o editor primeiro, não de um problema nas regras — resolvido selecionando tudo e apagando antes de colar.
- Não confirmado se o Cloud Storage já estava habilitado no projeto antes desta publicação (pode ter exigido o plano Blaze) — o usuário publicou direto pelo console sem reportar erro, então isso já deve estar resolvido do lado dele.

**Critério de saída:** código cliente completo; `storage.rules` e `firestore.rules` publicados no projeto real. Falta (1) teste comportamental real (upload como usuário Premium x gratuito — a publicação confirma que as regras compilam e estão no ar, não que a lógica de negação/permissão está correta na prática) e (2) teste manual ponta a ponta em dispositivo (mesma pendência de todas as fases anteriores).
